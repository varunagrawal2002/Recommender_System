...
for doc in documents_courses:
    ...
    author_names.append(author['Instructor_name'])
...
#Applying the KMeans Clustering Algorithm to form 3 clusters of courses
kmeans = KMeans(n_clusters=3)
kmeans.fit(course_encoded)

#Checking the query for the most relevant cluster of courses
query_cluster = kmeans.predict(encoded_query)
print(f"The query cluster is:\n{query_cluster}\n")
...
for i, course in enumerate(documents_courses):
    ...
    course_cluster = kmeans.labels_[i]
    if course_cluster == query_cluster:
